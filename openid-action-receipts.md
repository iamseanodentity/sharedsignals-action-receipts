---
title: OpenID SSF Action Receipts Extension 1.0

abbrev: action-receipts
docname: openid-action-receipts-1_0
date: 2026-05-05
...
This document defines an extension to the OpenID Shared Signals Framework (SSF) Specification 1.0 [SSF] that introduces a mechanism for SSF Receivers to send Action Receipts back to SSF Transmitters. An Action Receipt is a Security Event Token (SET) [RFC8417](#8417) sent by a Receiver to a Transmitter to indicate that one or more actions were taken in response to a previously received event. Action Receipts reference the originating event via its txn claim and carry freeform human-readable descriptions of the actions taken. This extension defines a new event type, a new Transmitter Configuration Metadata field for advertising the receipt endpoint, and an optional acknowledgement mechanism by which Transmitters may confirm receipt of an Action Receipt.

--- middle

# Introduction {#introduction}

The OpenID Shared Signals Framework [SSF](#SSF) defines a unidirectional flow of Security Event Tokens (SETs) from a Transmitter to one or more Receivers. While this model is well suited to event notification, it provides no native mechanism for a Receiver to communicate back to the Transmitter about actions it has taken in response to received events.

In many deployments, Transmitters benefit from knowing whether and how a Receiver has acted upon a signaled event — for example, whether a session was terminated, a user was prompted for re-authentication, or an access policy was enforced. This feedback enables Transmitters to make better downstream decisions, close audit loops, and satisfy compliance requirements.

This specification defines an Action Receipt mechanism for SSF. An Action Receipt is a SET originated by the Receiver and delivered to the Transmitter via a dedicated endpoint. It references the originating event by its txn claim, describes one or more actions taken by the Receiver in freeform text, and optionally includes structured metadata about each action. The Transmitter MAY optionally acknowledge receipt of an Action Receipt by sending its own acknowledgement SET back to the Receiver via the existing SSF event stream.
...
when, and only when, they appear in all capitals, as shown here. Joy!
